...
namespace wap {
namespace detail {
template <bool Output> struct BasicStreamHelper {
    static inline void memcpy(char *to, const char *from, size_t size) {
        ::memcpy(to, from, size);
    }
};
template <> struct BasicStreamHelper<true> {
    static void memcpy(const char *from, char *to, size_t size) {
        ::memcpy(to, from, size);
    }
};
template <bool Output> class BasicStream {
    typedef typename std::conditional<Output, char *, const char *>::type
        buffer_ptr_t;

  protected:
    inline BasicStream(buffer_ptr_t buffer, size_t buffer_size,
                       unsigned offset = 0)
  ...
  private:
    ...
    template <typename T> void read_write_impl(const T &val) {
        T tmp = val;
        read_write_impl(tmp);
        if (!Output && tmp != val)
            throw Error(WAP_EINVALIDDATA);
    }
    template <typename T> void read_write_impl(T &val) {
        if (m_offset > m_buffer_size - sizeof(T)) {
            throw wap::Error(WAP_EINVALIDDATA);
        }
        char *val_ptr = reinterpret_cast<char *>(&val);
        BasicStreamHelper<Output>::memcpy(val_ptr, m_buffer + m_offset,
                                          sizeof(T));
        if (wap::system_is_big_endian())
            std::reverse(val_ptr, val_ptr + sizeof(T));
        m_offset += sizeof(T);
    }
    template <typename T, size_t size> void read_write_impl(T(&buffer)[size]) {
...
class InputStream : public detail::BasicStream<false> {
  public:
    ...
    template <typename T> void read_buffer(T *buffer, size_t buffer_size) {
...
class OutputStream : public detail::BasicStream<true> {
  public:
    ...
    template <typename T> void write_buffer(T *buffer, size_t buffer_size) {
        read_write_buffer(buffer, buffer_size);
    }
};
