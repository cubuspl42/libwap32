...
{
    stream.read(header.id, header.flags, header.width, header.offset_x, header.offset_y);
}

void pid_read_header(wap_pid_header &header, const char *pid_buffer, size_t pid_buffer_size)
{
...
int wap_pid_read_header(wap_pid_header *header, const char *pid_buffer, size_t pid_buffer_size)
{
    return wap::handle_exceptions(pid_read_header, *header, pid_buffer, pid_buffer_size);
}
