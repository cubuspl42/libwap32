...
void decompress_buffer(char *out_buffer, size_t out_buffer_size,
...
static void read_string(wap::InputStream &stream, std::string &str,
                        size_t len) {
    str.resize(len);
    stream.read_buffer(&str[0], str.size());
}

static void read_nullterminated_string(wap::InputStream &stream,
                                       std::string &str) {
    str.erase();
    char c = '\0';
    while (1) {
...
static void read_objects(wap::InputStream &stream,
    ...
    for (wap_object &obj : objects) {
        ...
        stream.read(objp.id, name_len, logic_len, image_set_len, animation_len,
                    objp.x, objp.y, objp.z, objp.i, objp.add_flags,
                    objp.dynamic_flags, objp.draw_flags, objp.user_flags,
                    objp.score, objp.points, objp.powerup, objp.damage,
        ...
        stream.read(objp.x_min, objp.y_min, objp.x_max, objp.y_max,
                    objp.speed_x, objp.speed_y, objp.x_tweak, objp.y_tweak,
                    objp.counter, objp.speed, objp.width, objp.height,
                    objp.direction, objp.face_dir, objp.time_delay,
                    objp.frame_delay, objp.object_type, objp.hit_type_flags,
                    objp.x_move_res, objp.y_move_res);
...
static void read_planes(wap::InputStream &stream,
    ...
    for (wap_plane &plane : planes) {
        ...
        stream.read(160, 0, planep.flags, 0, planep.name, width_px, height_px,
                    ...
                    num_image_sets, num_objects, plane_offsets->tiles_offset,
                    plane_offsets->image_sets_offset,
                    plane_offsets->objects_offset, planep.z_coord, 0, 0, 0);
...
static void
read_tile_descriptions(wap::InputStream &stream,
...
static void
read_main_block(wap::InputStream &stream, std::vector<wap_plane> &planes,
                std::vector<wap_tile_description> &tile_descriptions,
                const wwd_offsets &offsets) {
...
static void read_header(wap::InputStream &stream, wap_wwd &wwd,
                        wwd_offsets &offsets,
    ...
    stream.read(0, wwdp.flags, 0, wwdp.level_name, wwdp.author, wwdp.birth,
                wwdp.rez_file, wwdp.image_dir, wwdp.pal_rez, wwdp.start_x,
                wwdp.start_y, unknown, num_planes, offsets.main_block_offset,
                offsets.tile_descriptions_offset, decompressed_main_block_size,
                wwd.checksum, 0, wwdp.launch_app, wwdp.image_sets[0],
                wwdp.image_sets[1], wwdp.image_sets[2], wwdp.image_sets[3],
                wwdp.prefixes[0], wwdp.prefixes[1], wwdp.prefixes[2],
                wwdp.prefixes[3]);

    wwd.planes.resize(num_planes);
}

void wwd_read(wap_wwd &out_wwd, const char *wwd_buffer,
              size_t wwd_buffer_size) {
    wap_wwd wwd;
    ...
    if (wwdp.flags & WAP_WWD_FLAG_COMPRESS) {
        offsets.eof_offset =
            offsets.main_block_offset + decompressed_main_block_size;
        std::vector<char> decompressed_buffer(offsets.eof_offset);
        memcpy(decompressed_buffer.data(), wwd_buffer,
               offsets.main_block_offset);
        const char *compressed_main_block =
            wwd_buffer + offsets.main_block_offset;
        size_t compressed_main_block_size =
            wwd_buffer_size - offsets.main_block_offset;
        char *decompressed_main_block =
            decompressed_buffer.data() + offsets.main_block_offset;
        decompress_buffer(decompressed_main_block, decompressed_main_block_size,
                          compressed_main_block, compressed_main_block_size);
        wap::InputStream stream_decompressed(
            &decompressed_buffer[0], decompressed_buffer.size(), stream.tell());
        read_main_block(stream_decompressed, wwd.planes, wwd.tile_descriptions,
                        offsets);
    } else {
...
int wap_wwd_read(wap_wwd *out_wwd, const char *wwd_buffer,
                 size_t wwd_buffer_size) {
    return wap::handle_exceptions(wwd_read, *out_wwd, wwd_buffer,
                                  wwd_buffer_size);
}
