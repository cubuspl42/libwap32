...
std::vector<char> compress_buffer(const char *in_buffer,
...
static void calculate_offsets(wwd_offsets &offsets,
    ...
    for (size_t i = 0; i < num_planes; ++i) {
        ...
        for (const wap_object &object : wwd.planes[i].objects) {
            ...
            offset += object.name.size() + object.logic.size() +
                      object.image_set.size() + object.animation.size();
        }
    }
...
// This function tries to imitate WapWorld's checksum function. It's result is
// different on all compressed and some
// non compressed files. That's ok, because as far as I know it's only used to
// check if two multiplayer clients have the same
// version of map installed. Partial compatiblity with WapWorld's checksum is
// just to allow comparing our wwd buffers with
...
static void write_nullterminated_string(wap::OutputStream &stream,
...
static void
write_main_block(wap::OutputStream &stream,
                 const std::vector<wap_plane> &planes,
                 const std::vector<wwd_plane_offsets> &planes_offsets,
                 const std::vector<wap_tile_description> &tile_descriptions) {
...
static void write_header(wap::OutputStream &stream, const wap_wwd &wwd,
    ...
    stream.write(0, wwdp.flags, 0, wwdp.level_name, wwdp.author, wwdp.birth,
                 wwdp.rez_file, wwdp.image_dir, wwdp.pal_rez, wwdp.start_x,
                 wwdp.start_y, 0, num_planes, offsets.main_block_offset,
                 offsets.tile_descriptions_offset, decompressed_main_block_size,
                 checksum, 0, wwdp.launch_app, wwdp.image_sets[0],
...
int wap_wwd_write(const wap_wwd *wwd, wap_buffer *out_wwd_buffer) {
    return wap::handle_exceptions(
        wwd_write, wwd, *wap::cast_wap_buffer_to_vector(out_wwd_buffer));
}
