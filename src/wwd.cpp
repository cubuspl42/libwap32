...
wap_wwd *wap_wwd_create() { return new (std::nothrow) wap_wwd; }

void wap_wwd_free(wap_wwd *wwd) { delete wwd; }

unsigned wap_wwd_get_checksum(const wap_wwd *wwd) { return wwd->checksum; }
...
int wap_wwd_set_plane_count(wap_wwd *wwd, unsigned count) {
    return wap::handle_exceptions([=]() { wwd->planes.resize(count); });
}
...
wap_tile_description *wap_wwd_get_tile_description(wap_wwd *wwd,
...
void wap_plane_set_tile(wap_plane *plane, unsigned x, unsigned y,
                        unsigned tile) {
...
int wap_plane_set_image_set_count(wap_plane *plane, unsigned count) {
    return wap::handle_exceptions([=]() { plane->image_sets.resize(count); });
}

const char *wap_plane_get_image_set(const wap_plane *plane,
                                    unsigned image_set_index) {
    return plane->image_sets[image_set_index].c_str();
}

int wap_plane_set_image_set(wap_plane *plane, unsigned image_set_index,
...
int wap_plane_set_object_count(wap_plane *plane, unsigned count) {
    return wap::handle_exceptions([=]() { plane->objects.resize(count); });
}
...
int wap_object_set_name(wap_object *object, const char *name) {
    return wap::handle_exceptions([=]() { object->name.assign(name); });
}
...
int wap_object_set_logic(wap_object *object, const char *logic) {
    return wap::handle_exceptions([=]() { object->logic.assign(logic); });
}
