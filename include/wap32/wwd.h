...
WAP_API int wap_wwd_read(wap_wwd *wwd, const char *wwd_buffer,
...
/* Get the checksum calculated by a program that created the WWD buffer that was
 * read using wap_wwd_read or wap_wwd_open.
...
WAP_API wap_tile_description *
wap_wwd_get_tile_description(wap_wwd *wwd, uint32_t description_index);

WAP_API wap_wwd_properties *wap_wwd_get_properties(wap_wwd *wwd);

WAP_API void wap_plane_get_map_dimensions(wap_plane *plane, uint32_t *w,
                                          uint32_t *h);

/* Note: after changing map's dimensions, value returned by
 * wap_plane_get_tile(plane, x, y) is undefined for any x, y until you
 * call wap_plane_set_tile(plane, x, y, tile) or read a new buffer with
 * wap_wwd_read or wap_wwd_open.
 * Possible error: WAP_ENOMEMORY
 */
WAP_API int wap_plane_set_map_dimensions(wap_plane *plane, uint32_t w,
                                         uint32_t h);

WAP_API uint32_t wap_plane_get_tile(const wap_plane *plane, uint32_t x,
                                    uint32_t y);

WAP_API void wap_plane_set_tile(wap_plane *plane, uint32_t x, uint32_t y,
                                uint32_t tile);
...
WAP_API const char *wap_plane_get_image_set(const wap_plane *plane,
                                            uint32_t image_set_index);

/* Possible error: WAP_ENOMEMORY */
WAP_API int wap_plane_set_image_set(wap_plane *plane, uint32_t image_set_index,
...
WAP_API wap_object *wap_plane_get_object(wap_plane *plane,
