...
int main(int argc, const char **argv)
{
    ...
    std::vector<char> wwd_buffer_loaded((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    
    wap_wwd *wwd = wap_wwd_create();
    assert(wwd);
    
    int error = 0;
    error = wap_wwd_read(wwd, wwd_buffer_loaded.data(), wwd_buffer_loaded.size());
    assert(!error);
    
    assert(!(wap_wwd_get_properties(wwd)->flags & WAP_WWD_FLAG_COMPRESS)); // It's best to test on non compressed files
    ...
    //error = wap_wwd_save(wwd, filename.replace(filename.size()-4, 4, "_libwap32.WWD").c_str());
    //assert(!error);
    
    assert(wwd_buffer_loaded == *wap::cast_wap_buffer_to_vector(&wwd_buffer_decompressed));
